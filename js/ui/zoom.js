...
export function initZoom() {
  ...
  let initialPinchLocalCX = 0, initialPinchLocalCY = 0;
  let lastLocalCX = 0, lastLocalCY = 0;
  ...
  function renderLoop() {
    ...
    if (pointers.length === 0 && (Math.abs(velocityX) > MOMENTUM_CUTOFF || Math.abs(velocityY) > MOMENTUM_CUTOFF)) {
      panX += velocityX;
      panY += velocityY;

...
export async function loadProjectHistory(projectId) {
  ...
  try {
    ...
    if (res.snapshots.length === 0) {
      list.innerHTML = '<div style="padding:20px; text-align:center; opacity:0.5;">Noch keine Snapshots für dieses Projekt.</div>';
      return;
    }
    ...
    res.snapshots.forEach(snap => {
      ...
      el.addEventListener('click', async () => {
        if (confirm(`Möchtest du Version ${snap.version} wiederherstellen? Aktuelle ungespeicherte Änderungen gehen verloren.`)) {

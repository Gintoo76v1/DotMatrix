...
import { toGrayscale, floydSteinberg, orderedDither, thresholdDither, boxBlur3x3 } from '../js/filters.js';
...
describe('toGrayscale (legacy RGB path)', () => {
  it('legacy path produces same result for grey images', () => {
    ...
    const luma   = toGrayscale(img, defaultGrayState({ contrast: 30, gamma: 1.5 }), { grayscale: 'luma' });
    const legacy = toGrayscale(img, defaultGrayState({ contrast: 30, gamma: 1.5 }), { grayscale: 'rgb' });
...
describe('floydSteinberg', () => {
  ...
  it('serpentine mode gives different output than classic (same input)', () => {
    ...
    const classic    = floydSteinberg(gray, w, h, { floydSteinberg: 'classic',    useFloydThreshold: false });
    const serpentine = floydSteinberg(gray, w, h, { floydSteinberg: 'serpentine', useFloydThreshold: false });
    let diff = 0;
...
describe('boxBlur3x3 (separable)', () => {
  it('constant image is unchanged', () => {
    ...
    boxBlur3x3(data, w, h);
    for (let i = 0; i < data.length; i++) expect(Math.abs(data[i] - orig[i])).toBeLessThanOrEqual(1);
  });
